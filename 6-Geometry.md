...
```c++
template<class P>
vector<P> minkowski(vector<P> C1, vector<P> C2){
	auto reorder_polygon = [](vector<P> &pts){
		size_t pos = 0;
	...
	vector<P> ret;
	size_t i = 0, j = 0;
	while (i < C1.size() - 2 || j < C2.size() - 2){
		ret.push_back(C1[i] + C2[j]);
		auto cross = (C1[i + 1] - C1[i]).cross(C2[j + 1] - C2[j]);
		if (cross >= 0 && i < C1.size() - 2)
			++i;
		if (cross <= 0 && j < C2.size() - 2)
			++j;
	}
	return ret;
}
